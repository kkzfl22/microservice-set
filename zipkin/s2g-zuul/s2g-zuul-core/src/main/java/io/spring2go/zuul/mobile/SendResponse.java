...
public class SendResponse extends ZuulFilter {
  private static final DynamicIntProperty BUFFER_SIZE =
      DynamicPropertyFactory.getInstance()
          .getIntProperty(Constants.ZUUL_INITIAL_STREAM_BUFFER_SIZE, 1024);
  private static final DynamicBooleanProperty CONTENT_LENGTH =
      DynamicPropertyFactory.getInstance()
          .getBooleanProperty(Constants.ZUUL_SET_CONTENT_LENGTH, false);
    ...
    try {
      ...
      if (isGZipRequested) {
        ...
        try {
          ...
          e.printStackTrace();
          // Cat.logError("gzip expected but not received assuming unencoded response" +
          // RequestContext.getCurrentContext().getRequest().getRequestURL().toString(),e);
        }
      }
      writeResponse(inStream, outStream);
  ...
  private void writeResponse(InputStream zin, OutputStream out) throws IOException {
    ...
    try {
      ...
      RequestContext.getCurrentContext()
          .set("sendResponseCost", System.currentTimeMillis() - start);
