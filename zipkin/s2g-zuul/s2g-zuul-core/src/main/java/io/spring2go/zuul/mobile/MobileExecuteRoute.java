...
public class MobileExecuteRoute extends ZuulFilter {
  private static final DynamicIntProperty maxConnection =
      DynamicPropertyFactory.getInstance()
          .getIntProperty(Constants.ZUUL_CLIENT_MAX_CONNECTIONS, 500);
  private static final DynamicIntProperty maxRouteConnection =
      DynamicPropertyFactory.getInstance()
          .getIntProperty(Constants.ZUUL_CLIENT_ROUTE_MAX_CONNECTIONS, 20);

  private static final Runnable loader =
      new Runnable() {
        @Override
        public void run() {
  ...
  private static final AtomicReference<CloseableHttpClient> clientRef =
      new AtomicReference<CloseableHttpClient>(newClient());
  ...
  public Object run() throws ZuulException {
    ...
    try {
      ...
      //      Cat.logError(
      //          String.format(
      //              "Service Execution Error,OriginUrl: %s\nTargetUrl: %s\nTargetIp: %s\nCause:
      // %s\n,groupName:%s\n,routeName:%s",
      //              originUrl, targetUrl, targetIp, errorMsg, groupName, routeName),
      //          e);
      // throw new ZuulException(errorMsg,500, "TargetUrl: ${targetUrl}\nCause: ${errorMsg}");
      throw new ZuulException(
          errorMsg, 500, String.format("TargetUrl: %s\nCause: %s", targetUrl, errorMsg));
    }
    ...
    if (Debug.debugRequest()) {
      if (inputStream == null) {
        ...
        if (isOriginResponseGZipped) {
          contentBytes =
              getBytes(new GZIPInputStream(new ByteArrayInputStream(contentBytes)), 8096);
        }
  ...
  public HttpResponse forward(
    ...
    try {
      if ("THREAD".equalsIgnoreCase(isolationStrategy)) {
        return new ZuulRequestCommandForThreadIsolation(
                httpclient, httpUriRequest, groupName, routeName)
            .execute();
      } else {
        return new ZuulRequestCommandForSemaphoreIsolation(
                httpclient, httpUriRequest, groupName, routeName)
            .execute();
      }
    } finally {
  ...
  private InputStream debugRequestEntity(InputStream inputStream, int contentLength)
      throws IOException {
  ...
  private RequestConfig buildRequestConfig(String routName, String groupName) {
    ...
    int connectTimeout =
        DynamicPropertyFactory.getInstance().getIntProperty(routName + ".connect.timeout", 0).get();
    if (connectTimeout == 0) {
      connectTimeout =
          DynamicPropertyFactory.getInstance()
              .getIntProperty(groupName + ".connect.timeout", 0)
              .get();
    }
    if (connectTimeout == 0) {
      connectTimeout =
          DynamicPropertyFactory.getInstance()
              .getIntProperty("zuul.connect.timeout.global", 2000)
              .get();
    }
    builder.setConnectTimeout(connectTimeout);

    int socketTimeout =
        DynamicPropertyFactory.getInstance().getIntProperty(routName + ".socket.timeout", 0).get();
    if (socketTimeout == 0) {
      socketTimeout =
          DynamicPropertyFactory.getInstance()
              .getIntProperty(groupName + ".socket.timeout", 0)
              .get();
    }
    if (socketTimeout == 0) {
      socketTimeout =
          DynamicPropertyFactory.getInstance()
              .getIntProperty("zuul.socket.timeout.global", 1000)
              .get();
    }
    ...
    int requestConnectionTimeout =
        DynamicPropertyFactory.getInstance()
            .getIntProperty(routName + ".request.connection.timeout", 0)
            .get();
    if (requestConnectionTimeout == 0) {
      requestConnectionTimeout =
          DynamicPropertyFactory.getInstance()
              .getIntProperty(groupName + ".request.connection.timeout", 0)
              .get();
    }
    if (requestConnectionTimeout == 0) {
      requestConnectionTimeout =
          DynamicPropertyFactory.getInstance()
              .getIntProperty("zuul.request.connection.timeout.global", 10)
              .get();
    }
