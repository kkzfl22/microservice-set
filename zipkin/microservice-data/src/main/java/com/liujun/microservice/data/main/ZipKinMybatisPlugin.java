...
public class ZipkinMybatisPlugin implements Interceptor {

  private static Log logger = LogFactory.getLog(ZipkinMybatisPlugin.class);
  ...
  private Tracer tracer = DataSpringApplicationContext.getBean(Tracer.class);
  ...
  public Object intercept(Invocation invocation) throws Throwable {
    ...
    Span span = tracer.createSpan("mybatis");
    ...
    span.logEvent("run sql:" + sql);

    span.tag("tage SQL.Method", sqlCommandType.name().toLowerCase());

    String s = this.getSQLDatabase();
    // Cat.logEvent("SQL.Database", s);
    // span.logEvent("SQL.Database :" + s);
    span.tag("tage SQL.Database", s);
  ...
  private javax.sql.DataSource getDataSource() {
    ...
    if (transaction instanceof SpringManagedTransaction) {
      ...
      Field field =
          ReflectionUtils.findField(transaction.getClass(), fieldName, javax.sql.DataSource.class);
      ...
      javax.sql.DataSource dataSource =
          (javax.sql.DataSource) ReflectionUtils.getField(field, transaction);
      return dataSource;
    }
  ...
  private String getSqlURL() {
    ...
    if (dataSource instanceof AbstractRoutingDataSource) {
      ...
      if (method == null) {
        logger.error(
            String.format("---Could not find method [%s] on target [%s]", methodName, dataSource));
        return null;
      }

      ReflectionUtils.makeAccessible(method);
      javax.sql.DataSource dataSource1 =
          (javax.sql.DataSource) ReflectionUtils.invokeMethod(method, dataSource);
  ...
  private String getSQLDatabase() {
    ...
    String url = ZipkinMybatisPlugin.sqlURLCache.get(dbName);
    if (url != null) {
      return url;
    }
    ...
    ZipkinMybatisPlugin.sqlURLCache.put(dbName, url);
    return url;
  }
  /**
   * 解析sql语句
