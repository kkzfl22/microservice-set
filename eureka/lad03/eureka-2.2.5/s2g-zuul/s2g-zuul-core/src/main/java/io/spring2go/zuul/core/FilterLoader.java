...
import io.spring2go.zuul.mobile.ExecuteRibbon;
import io.spring2go.zuul.mobile.TestRibbonRouting;
...
/**
 * This class is one of the core classes in Zuul. It compiles, loads from a File, and checks if
 * source code changed. It also holds ZuulFilters by filterType.
 */
public class FilterLoader {
  ...
  private final ConcurrentHashMap<String, String> filterCheck =
      new ConcurrentHashMap<String, String>();
  private final ConcurrentHashMap<String, List<ZuulFilter>> hashFiltersByType =
      new ConcurrentHashMap<String, List<ZuulFilter>>();

  public FilterLoader() {
    //        filterRegistry.put("route3",new ExecuteRibbon());
    filterRegistry.put("routeRibbon ", new TestRibbonRouting());
  ...
  /**
   * Given source and name will compile and store the filter if it detects that the filter code has
   * changed or the filter doesn't exist. Otherwise it will return an instance of the requested
   * ZuulFilter
   *
  ...
  /**
   * From a file this will read the ZuulFilter source code, compile it, and add it to the list of
   * current filters a true response means that it was successful.
   *
   * @param file
